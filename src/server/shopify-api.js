...
var SHAPI = function(authData){
	return this.setAuth(authData);
};

SHAPI.prototype = (function(){
	var shopify = null;
	/**
	 * Auth
	 *
	...
	const setAuth = (authorization) => {
		shopify = new Shopify(authorization);
		return shopify;
	};
	 ...
	 * - getters
	 */
	const getCollectionById = (collection_id, number, fields, callback) => {
		if(!callback){
			callback = noop;
		}
	...
	const getCollectionByName = (collection_name, fields, callback) => {
		if(!callback){
			callback = noop;
		}
	 ...
	 *  - setters
	 */
	const setNewCollectionName = (collection_name, callback) => {
		if(!callback){
			callback = noop;
		}
	...
	const addProductToCollection = (collection_id, product_id, callback) => {
		if(!callback){
			callback = noop;
		}
	...
	const getProductsCollection = (collection_id, number, fields, callback) => {
	 ...
	 *  - setters
	 */
	const setProduct = (product_options, collectionId, callback) => {
		if(!callback){
			callback = noop;
		}
